    ...
    const tabOrientationTranslation: tabTranslationType = {
      startLobe: {
        top: {
          ...
          x: start,
          y: base + edge.lineFunction.call(start) + tab.startCurveDiameter, // +curveconstant for down, - for up
        },
        bottom: {
          cpx: start - tab.endCurveDiameter,
          cpy: base + edge.lineFunction.call(start - tab.endCurveDiameter),
          x: start,
          y: base + edge.lineFunction.call(start) + tab.endCurveDiameter,
        },
        right: {
          cpx: base + edge.lineFunction.call(start + tab.startCurveDiameter),
          cpy: start + tab.startCurveDiameter,
          x: base + edge.lineFunction.call(start) + tab.startCurveDiameter,
          y: start,
        },
        left: {
          cpx: base + edge.lineFunction.call(start - tab.startCurveDiameter),
      ...
      middleLobe: {
        top: {
          ...
          cp1y: base + edge.lineFunction.call(start) + tab.mainLobeHeight,
          cp2y: base + edge.lineFunction.call(start) + tab.mainLobeHeight,
          x: end,
          y: base + edge.lineFunction.call(end) + tab.endCurveDiameter,
        },
        bottom: {
          cp1x: start + tab.mainLobeWidth,
          cp2x: end - tab.mainLobeWidth,
          ...
          cp2y: base + edge.lineFunction.call(start) + tab.mainLobeHeight,
          x: end,
          y: base + edge.lineFunction.call(end) + tab.startCurveDiameter,
        },
        right: {
          cp1x: base + edge.lineFunction.call(start) + tab.mainLobeHeight,
          cp2x: base + edge.lineFunction.call(start) + tab.mainLobeHeight,
          cp1y: start - tab.mainLobeWidth,
          cp2y: end + tab.mainLobeWidth,
          x: base + edge.lineFunction.call(end) + tab.endCurveDiameter,
          y: end,
        },
        left: {
          cp1x: base + edge.lineFunction.call(start) + tab.mainLobeHeight,
          ...
          cp1y: start + tab.mainLobeWidth,
          cp2y: end - tab.mainLobeWidth,
          x: base + edge.lineFunction.call(end) + tab.startCurveDiameter,
          y: end,
        }
      },
      endLobe: {
